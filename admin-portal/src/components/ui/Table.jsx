export default function Table({ columns, rows, keyField = 'id', emptyMessage = 'No data yet.', rowClassName }) {
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left text-sm">
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row[keyField]} className={`border-b border-slate-100 last:border-0 hover:bg-slate-50 ${rowClassName ? rowClassName(row) : ''}`}>
              {columns.map((col) => (
