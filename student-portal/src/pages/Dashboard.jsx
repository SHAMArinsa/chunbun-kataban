...
export default function Dashboard() {
  ...
  const { data: documents } = useQuery({ queryKey: ['student-documents', 'me'], queryFn: () => apiClient.get('/student-documents/me').then((r) => r.data) })
  ...
  return <div className="student-dashboard"><section className="dashboard-hero"><div><p className="dashboard-kicker"><Sparkles size={15} /> ARINSA STUDENT PORTAL</p><h1>Welcome back,<br /><em>Student.</em></h1><p>{summary}</p><a href="/materials">Continue learning <ArrowRight size={18} /></a></div><div className="dashboard-progress"><span>YOUR JOURNEY</span><strong>{activeEnrollment ? `Week ${activeEnrollment.current_week || 1}` : 'Ready to begin'}</strong><p>{activeEnrollment ? 'Stay consistent—your next milestone is waiting.' : 'Choose a programme to begin your journey.'}</p><div><i style={{ width: activeEnrollment ? `${Math.min((activeEnrollment.current_week || 1) * 10, 100)}%` : '8%' }} /></div></div></section><section className="dashboard-stats"><StatCard icon={GraduationCap} label="Enrollments" value={enrollments?.length ?? 0} /><StatCard icon={ClipboardCheck} label="Quizzes passed" value={attempts?.filter((a) => a.passed).length ?? 0} /><StatCard icon={Award} label="Documents" value={documents?.length ?? 0} /><StatCard icon={Clock} label="Current week" value={activeEnrollment?.current_week ?? '—'} /></section>{isBasic && <BasicGuide />}{isProfessional && <ProfessionalGuide />}{isPremium && <PremiumGuide />}{isPlatinum && <PlatinumGuide />}<section className="dashboard-enrollments"><div className="dashboard-section-title"><div><p>YOUR PROGRAMMES</p><h2>Learning workspace</h2></div><a href="/materials">View materials <ArrowRight size={16} /></a></div>{enrollments?.length ? <div className="dashboard-enrollment-list">{enrollments.map((e) => <article key={e.id}><div><span>PROGRAMME</span><h3>{e.program_name || programmeNames[e.program_id] || `Program #${e.program_id}`}</h3><p>Enrolled {new Date(e.enrolled_at).toLocaleDateString()}</p></div><div className="dashboard-enrollment-status"><Badge color={e.status === 'active' ? 'green' : e.status === 'pending_payment' ? 'yellow' : 'slate'}>{e.status.replace('_', ' ')}</Badge><a href="/timeline" aria-label="Open programme"><ArrowRight size={19} /></a></div></article>)}</div> : <div className="dashboard-empty"><GraduationCap size={24} /><h3>Your workspace is waiting.</h3><p>Choose an internship programme to begin.</p><a href="/signup">Explore programmes <ArrowRight size={17} /></a></div>}</section></div>
}
